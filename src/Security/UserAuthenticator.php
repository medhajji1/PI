...
class UserAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        $this->logger->info('I just got the logger22222 ');
        $this->logger->info($request->request->get('password', ''));
        return new Passport(
            new UserBadge($email),
    ...
    {  
       ...
       if ($targetPath = $this->getTargetPath($request->getSession(), $firewallName)) {
        ...
        if($user->getRoles()[0]=="Client"){
            ...
            return new RedirectResponse($this->urlGenerator->generate('list_utilisateur'));
    ...
    {
        $this->logger->info('Failure login');
